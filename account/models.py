...
class CustomAccountManager(BaseUserManager):
    ...
    def create_superuser(self,email,user_name,password,**other_fields):
        ...
        if other_fields.get('is_staff') is not True:
            raise ValueError('Superuser must be a part of staff')
        
        if other_fields.get('is_active') is not True:
            raise ValueError('Superuser must be an active user')
        
        if other_fields.get('is_superuser') is not True:
            raise ValueError('is_superuser field for superuser must be set to true')
        
        return self.create_user(email,user_name,password,**other_fields)
    
    def create_user(self,email,user_name,password,**other_fields):
        if not email:
            raise ValueError('Email address is mandatory! Please provide one...')
...
class UserBase(AbstractBaseUser,PermissionsMixin):
    ...
    def email_user(self,subject,message):
        send_mail(subject, message, 'abc@gmail.com',[self.email],fail_silently=False)
